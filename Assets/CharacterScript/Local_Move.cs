...
public class Local_Move : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)   // 그라운드 태그 거나 카메라 스왑 태그 지형이면 카메라 스왑
    {
        ...
        else if (collision.gameObject.CompareTag("Slide")) // 슬라이딩 지형이면 슬라이딩 애니메이션 
        {
            isGrounded = true;
            isJumping = false;
            isDoubleJumping = false;
